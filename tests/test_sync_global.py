...
@patch("sync_global.subprocess.run")
def test_push_global_memory_with_changes(mock_run, mock_git_cmd, mock_global_root):
    # Mock .git exists
    mock_global_root.__truediv__.return_value.exists.return_value = True
...
def test_share_local_record_success(mock_copy, mock_reviews_dir, mock_global_root):
