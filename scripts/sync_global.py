...
def cmd_init(args):
    ...
    # Use subprocess.run directly instead of run_git_cmd to capture
    # interactive cloning prompts if needed
...
def pull_global_memory():
    ...
    if not (global_dir / ".git").exists():
        raise Exception(
            "未找到全局记忆库。请先使用 `python scripts/sync_global.py init <repo_url>` 初始化。")

    subprocess.run(["git", "pull", "--rebase"],
                   cwd=str(global_dir), check=True)
    return "成功拉取最新全局记忆"
...
def push_global_memory():
    ...
    status_output = run_git_cmd(
        ["git", "status", "--porcelain"], cwd=global_dir)
    ...
    run_git_cmd(["git",
                 "commit",
                 "-m",
                 "chore(memory): auto-sync global memory"],
                cwd=global_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync global memory records across projects via Git.")
    subparsers = parser.add_subparsers(
        dest="command", required=True, help="Subcommands")

    # init
    parser_init = subparsers.add_parser(
        "init", help="Initialize global memory by cloning a git repository.")
    parser_init.add_argument(
        "repo_url",
        type=str,
        help="Git repository URL (e.g., git@github.com:user/global-memory.git)")

    # pull
    parser_pull = subparsers.add_parser(
        "pull", help="Pull latest global memory from remote.")

    # push
    parser_push = subparsers.add_parser(
        "push", help="Push local global memory changes to remote.")

    # share
    parser_share = subparsers.add_parser(
        "share", help="Share a local project review to the global memory directory.")
    parser_share.add_argument(
        "review_id",
        type=str,
        nargs="+",
        help="The ID(s) of the local review record(s)")
