...
def load_record(review_id: str, reviews_dir: Path) -> dict:
    ...
    if not file_path.exists():
        print(
            f"Error: Record {review_id} not found at {file_path}",
            file=sys.stderr)
        sys.exit(1)
...
def cmd_update(args, reviews_dir: Path):
    ...
    if field in string_fields and append:
        print(
            f"Error: Cannot append to a string field '{field}'.",
            file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage local memory records (Update & Archive).")
    subparsers = parser.add_subparsers(
        dest="command", required=True, help="Subcommands")

    # Archive command
    parser_archive = subparsers.add_parser(
        "archive", help="Archive a memory record.")
    parser_archive.add_argument(
        "review_id",
        type=str,
        nargs="+",
        help="The ID(s) of the review record (e.g., REV-20260417-001 REV-20260417-002)")

    # Update command
    parser_update = subparsers.add_parser(
        "update", help="Update a field in a memory record.")
    parser_update.add_argument(
        "review_id",
        type=str,
        nargs="+",
        help="The ID(s) of the review record")
    parser_update.add_argument(
        "--field",
        "-f",
        type=str,
    ...
    parser_update.add_argument(
        ...
        type=str,
        required=True,
        help="New value to set or append")
    parser_update.add_argument(
        "--append",
        "-a",
        action="store_true",
        help="Append to an array field instead of overwriting")
