...
def migrate_v1_0_to_v1_1(data: dict) -> dict:
    ...
    new_data["timestamp"] = metadata.get(
...
def process_file(file_path: Path, backup_dir: Path, dry_run: bool = False):
    ...
    if dry_run:
        ...
        file_path.write_text(
            ...
            encoding="utf-8")
        print(
            f"[{file_path.name}] Successfully migrated. Original backed up to {backup_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Offline migration tool for memory records.")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print migrated JSON without modifying files.")
    ...
    backup_dir = REVIEWS_DIR.parent / "backups" / \
    ...
    print(
        f"Migration complete. Processed {success_count}/{len(files)} files successfully.")
