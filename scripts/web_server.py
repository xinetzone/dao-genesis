...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed_path.path.startswith(
                '/api/reviews/') and parsed_path.path.endswith('/archive'):
        ...
        # API: /api/reviews/{id}/update
            ...
            if not field or not value:
                ...
                self.wfile.write(json.dumps(
                    {"error": "Missing field or value"}).encode())
                return
            ...
            try:
                ...
                if field in array_fields:
                    ...
                    if append:
                        raise ValueError(
                            f"Cannot append to string field '{field}'")
                    data[field] = value
        ...
        # API: /api/sync/pull
            ...
            try:
                ...
                self._set_headers()
                self.wfile.write(json.dumps(
                    {"success": True, "message": msg}).encode())
            except Exception as e:
                self._set_headers(500)
                self.wfile.write(json.dumps({"error": str(e)}).encode())
            return
        ...
        # API: /api/sync/share
            ...
            if not review_id:
                self._set_headers(400)
                self.wfile.write(json.dumps(
                    {"error": "Missing review_id"}).encode())
                return
            try:
