...
def search_index(query: str, limit: int = 5, include_archived: bool = False):
    ...
    if not index_file.exists():
        print(
            f"Error: Search index not found at {index_file}.",
            file=sys.stderr)
        print(
            "Please run `python scripts/build_memory_cache.py` first.",
            file=sys.stderr)
        sys.exit(1)
    ...
    for entry in index_data:
        ...
        # Basic scoring: check if query is in task_type, conclusions, or
        # action_items
        score = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Interactive CLI tool to search memory records.")
    parser.add_argument(
        "query",
        type=str,
        nargs="?",
        help="Search query (keywords). If not provided, enters interactive mode.")
    parser.add_argument(
        "--limit",
        "-l",
        type=int,
    ...
    parser.add_argument("--all", "-a", action="store_true",
                        help="Include archived records in search")
    parser.add_argument(
        "--verbose",
        "-v",
        action="store_true",
        help="Print full JSON for matching records")
    ...
    if args.query:
        ...
        while True:
            try:
                ...
                results = search_index(
                    query, limit=args.limit, include_archived=args.all)
                display_results(results, verbose=args.verbose)
