...
我们提供了一个开箱即用的轻量级 Web 界面，让你在浏览器中直观地浏览、搜索和分析所有的复盘记录。无需安装 Node.js 等复杂依赖。

**启动服务：**
```bash
python scripts/web_server.py
```
终端会输出 `Serving at http://localhost:8000`。
打开浏览器访问该地址，你将看到一个包含：
- **实时搜索与筛选**：支持按关键词、状态快速定位记录。
- **记录详情与编辑**：左侧选定记录后，右侧展示完整结构，并支持一键**归档**或直接向列表中**追加新项**（如补充 `action_items`）。
- **可视化分析图表**：切换至 Analytics 面板，可查看 `task_type` 分布与按月沉淀数量趋势（基于 ECharts）。
