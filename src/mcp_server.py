...
@mcp.tool()
def search_reviews(query: str, limit: int = 5, include_archived: bool = False) -> str:
    """
    ...
    """
    try:
        results = search_index(query, limit=limit, include_archived=include_archived)
        if not results:
        ...
        for i, entry in enumerate(results, 1):
            ...
            output.append(f"{i}. {review_id}{status_tag} ({entry.get('timestamp', 'Unknown Date')})")
...
@mcp.tool()
def update_review(review_id: str, field: str, value: str, append: bool = False) -> str:
    """
    ...
    """
    array_fields = {"participants", "decisions", "success_factors", "failure_reasons", "best_practices", "action_items", "tags"}
